...
The International Astronomical Search Collaboration (IASC) is a citizen-science program in which teams inspect astronomical image sets and submit validated measurements through an official campaign workflow. NASA lists IASC as a NASA Science citizen-science project, and the IASC collaborators page lists NASA among its United States collaborators. IASC campaigns are designed so students and non-specialists can participate after training, usually by inspecting four-image data packets with Astrometrica.
...
For beginners, the most practical workflow is to open `*_report.txt` and `*_candidates.png`, inspect candidates marked `STRONG` first, then continue to `MODERATE` and `WEAK` only if there is time.
